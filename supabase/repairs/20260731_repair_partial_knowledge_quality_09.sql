...
create temporary table repair09_preserved_relation_ids
on commit drop
as
select id, to_jsonb(relations) as row_snapshot
from public.document_relations as relations;

create temporary table repair09_preserved_audit_ids
on commit drop
as
...
begin
    ...
    if exists (
        select preserved.id
        from pg_temp.repair09_preserved_relation_ids as preserved
    ...
    if exists (
        select preserved.id
        from pg_temp.repair09_preserved_audit_ids as preserved
