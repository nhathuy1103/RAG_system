-- Repair a very specific partially-applied migration 08 without dropping data.
--
-- Observed pre-repair state:
--   * public.document_relations and public.knowledge_quality_audit exist;
--   * their columns, indexes, RLS, policies and audit trigger match migration 08;
--   * four owner-scoping foreign keys are missing;
--   * migration-08 columns on public.documents are missing;
--   * public.ingestion_jobs.claim_token and its fenced check are missing;
--   * claim_ingestion_job(text, integer) has the pre-fencing result shape;
--   * migrations 09 and 10 have not been applied.
--
-- Before running:
--   1. Stop API/worker traffic.
--   2. Take and verify a database backup.
--   3. Run supabase/diagnostics/08_partial_schema_audit.sql and retain its output.
--
-- This script preserves existing relation/audit row IDs, validates current data,
-- repairs in place, and rolls back the entire transaction on any mismatch.
...
select pg_advisory_xact_lock(
    hashtext('agentic-rag:repair-partial-knowledge-quality-08')
);

do $repair$
begin
    if to_regclass('public.ingestion_control') is not null
       or exists (
           select 1
       ...
       or to_regclass('public.documents_active_normalized_identity_key') is not null
       or to_regclass('public.documents_canonical_family_version_key') is not null
       or to_regclass('public.documents_one_current_canonical_per_family') is not null
       or to_regclass('public.document_chunks_simhash_band_1_idx') is not null
       or exists (
        select 1
        from information_schema.columns
        where table_schema = 'public'
          and (
              (table_name = 'ingestion_jobs' and column_name in (
                  'completion_disposition',
                  'repair_request_key'
              ))
              or (
                  table_name = 'knowledge_quality_audit'
       ...
       or to_regprocedure(
        ...
        raise exception
            'Repair preflight failed: migration 09 or 10 is already present; refusing to downgrade later RPCs';
    end if;
...
create temporary table repair08_preserved_relation_ids
on commit drop
as
select id, to_jsonb(relations) as row_snapshot
from public.document_relations as relations;

create temporary table repair08_preserved_audit_ids
on commit drop
as
select id, to_jsonb(audit) as row_snapshot
...
-- Reassert execution grants for the already-present migration-08 RPCs.
...
begin
    ...
    if to_regprocedure(
       ...
       or to_regprocedure(
        ...
        raise exception
            'Repair postcondition failed: one or more migration-08 RPCs are missing';
    end if;

    if exists (
        select preserved.id
        from pg_temp.repair08_preserved_relation_ids as preserved
        left join public.document_relations as current_rows
          on current_rows.id = preserved.id
        where current_rows.id is null
           or to_jsonb(current_rows) is distinct from preserved.row_snapshot
    ) then
        raise exception
            'Repair postcondition failed: a pre-existing relation row was lost';
    end if;

    if exists (
        select preserved.id
        from pg_temp.repair08_preserved_audit_ids as preserved
        left join public.knowledge_quality_audit as current_rows
          on current_rows.id = preserved.id
        where current_rows.id is null
           or to_jsonb(current_rows) is distinct from preserved.row_snapshot
    ) then
        raise exception
            'Repair postcondition failed: a pre-existing audit row was lost';
    end if;
end;
$repair$;
